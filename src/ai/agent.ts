...
// エージェントの本体（プロンプト＋ツール＋実行設定）。
// HTTPルート（route.ts）とエージェント評価テスト（tests/evals）の両方がこれを使うことで、
// 「テストしているものが本番と同一」を保証する
...
export function buildInstructions() {
  ...
  // 公式ガイドの推奨に基づき、XMLスタイルのタグで「役割・動的データ・ツール方針・判断ルール・
  // 安全ルール・応答スタイル」を分離している
  return `<role>
...
export function runAgent({
  ...
  return streamText({
    ...
    // 503時の内部リトライは1回まで（モデル間の切替はai-fallbackが担う）
    maxRetries: 1,
